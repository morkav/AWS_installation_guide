...
[program:name\_wsgi\_process]\\
command=/home/ubuntu/.pyenv/versions/name\_project\_env/bin/gunicorn \\
\hspace*{0.2in}name\_project.wsgi:application -b 127.0.0.1:8005 -w 4 -t 30\\
environment=LANG="en\_US.UTF-8"\\
directory=/home/ubuntu/name\_project/name\_project/\\
user=name\_of\_user (ubuntu in general)\\
autostart=True\\
autorestart=True\\
stdout\_logfile=/var/log/django.log\\
stderr\_logfile=/var/log/django.log\\
...
server\_names\_hash\_bucket\_size 350;

underscores\_in\_headers on;

server {
    listen 80;
    server\_name project\_name
    client\_max\_body\_size 10m;
    set \$static\_root "/path/to/static/files";
    set \$media\_root "/path/to/media/files";
    ...
    location /media {
     ...
     location / {
        ...
        proxy\_pass http://127.0.0.1:8005;
        proxy\_read\_timeout 120;
        proxy\_connect\_timeout 120;
    }
}
\item run cd /etc/nginx/sites-enabled/
\item if there is a default file in it, remove it (sudo rm default)
\item creation of a symbolic link to our file : sudo ln -s /etc/nginx/sites-available/django django
\item to launch the server type : sudo service nginx start
\end{itemize}

\item every time you modify something, the dev server relaunch by itself, here it is not the case. You have to type : 
sudo supervisorctl restart name\_wsgi\_process
